...
<en-export export-date="20121219T054005Z" application="Evernote/Tomboy" version="4.x">"""
    ...
    public void init() {
        xsltConverter = new ConvertTomboyToEverNote()
        xsltConverter.init()
    ...
    static public void main(String ...args) {
        ...
        converter.processNotes(converter.sourceDirectory)
    ...
    public void processNotes(String srcDir) {
        ...
        new File(srcDir).eachFileMatch(notePattern) { f ->
            ...
            Note n = fetchNote(f)
            String convertedContent = xsltConverter.transformNoteToString(f)
            // make hyperlinks
       ...
       // xml.'en-export'('export-date': '20121214T054005Z', 'application':"Evernote/Tomboy", 'version':'4.x') {
            xml.note() {
               ...
               // content(xml.mkp.yieldUnescaped(preContentMarkup) + tomNote.text + xml.mkp.yieldUnescaped(endContentMarkup))
                ...
                // updated(tomNote.lastChangeDate.format("yyyyMMddThhmmssZ"))   //yyyyMMddhhmmss
